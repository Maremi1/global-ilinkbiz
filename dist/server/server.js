...
export default {
  async fetch(req) {
    // Simple SPA handler: always return index.html so client-side router can handle routes.
    return serveIndex();
  }
};
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-BD80Hzj0.js");
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./assets/router-BNLm49L6.js"),
